...
GameObject::GameObject(glm::vec3 _position)
{
    ...
    m_GShader = ShaderLoader::CreateShader("Normals3D.vert", "gBuffer.frag");
}
...
void GameObject::Draw()
{
    if (m_Mesh || m_SkinnedMesh)
    {
        ...
        if (m_ShaderLocation.fragShader == "BlinnFong3D_Deferred.frag")
        {
            SetBlinnFong3DUniforms();
            SetRimLighingUniforms();
            SetBlinnFong3DDeferredUniforms();
        }
...
void GameObject::DrawToGBuffer()
{
    glUseProgram(m_GShader);
    ShaderLoader::SetUniformMatrix4fv(std::move(m_GShader), "PVMMatrix", Statics::ActiveCamera.GetPVMatrix() * m_Transform.transform);
...
void GameObject::SetHeightmapShadowsUniforms()
{
    ShaderLoader::SetUniformMatrix4fv(std::move(m_ShaderID), "LightVPMatrix", ShadowMap::GetInstance().GetLightVPMatrix());
    ShaderLoader::SetUniformMatrix4fv(std::move(m_ShaderID), "Model", m_Transform.transform);
}
...
void GameObject::SetgBufferUniforms()
{
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, m_ActiveTextures[0].ID);
    ShaderLoader::SetUniform1i(std::move(m_GShader), "Albedo", 0);

    ShaderLoader::SetUniform1f(std::move(m_GShader), "SpecularStrength", 1.0f);
}
